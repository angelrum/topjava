...
public class MealRestController extends AbstractMealController {
    ...
    @GetMapping("/filter")
    public List<MealTo> getByFilter(@RequestParam @DateTimeFormat(iso = ISO.DATE_TIME) LocalDateTime start,
    ...
    public List<MealTo> getBetween(@RequestParam(required = false) @DateTimeFormat(iso = ISO.DATE) LocalDate startDate,
                                   @RequestParam(required = false) @DateTimeFormat(iso = ISO.TIME) LocalTime starTime,
                                   @RequestParam(required = false) @DateTimeFormat(iso = ISO.DATE) LocalDate endDate,
                                   @RequestParam(required = false) @DateTimeFormat(iso = ISO.TIME) LocalTime endTime) {
        return super.getBetween(startDate, starTime, endDate, endTime);
    }
}